...
public class SimpleOracleAutoIncreaseProvider implements OracleAutoIncreaseProvider {
    ...
    public void buildAutoIncrease(TableBuildInfo table, ColumnMeta autoColumn, List<String> sqlList) {
        ...
        String $sql = "CREATE OR REPLACE TRIGGER %s_increase BEFORE \n";
        $sql += "insert ON %s FOR EACH ROW \n";
        $sql += "begin \n";
        $sql += "select %s_sequence.nextval into:New.%s from dual; \n";
        $sql += "end;";

        sqlList.add(String.format($sql,table.getName(),table.getName(),table.getName(),autoColumn.getName()).toLowerCase());
